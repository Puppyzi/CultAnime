...
Playback uses signed same-origin HLS URLs so private Jellyfin addresses are never exposed to the browser. Requests arriving through Cloudflare use a 6 Mbps video and 192 Kbps audio ceiling by default to leave tunnel bandwidth headroom; direct LAN/container requests retain the higher Jellyfin ceiling. Override the public profile with `JELLYFIN_PUBLIC_VIDEO_BITRATE` and `JELLYFIN_PUBLIC_AUDIO_BITRATE` when the server's upload capacity has been measured. The player retries failed fragments, rebuilds a persistently stalled stream, and resumes at the previous playback position.
