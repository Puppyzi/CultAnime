...
function scoreAnilistCandidate(media, kind, syncContext = {}) {
  ...
  if (Number.isFinite(seasonNumber) && seasonNumber > 1) {
    ...
    if (seasonMarkerMatches(titleText, seasonNumber)) score += 80;

    if (
      syncContext.productionYear &&
      Number(media?.seasonYear) === Number(syncContext.productionYear)
    ) {
      score += 30;
    }
...
async function anilistDataForItem(item, kind, syncContext = {}) {
  const queries = kind === 'movie'
    ? [cleanSeriesNameForSearch(item.Name) || item.Name]
  ...
  const bestMatch = bestAnilistCandidate(candidates, kind, syncContext);
...
function shouldRefreshExistingAnimeMetadata(existingRecord, syncContext = {}) {
  ...
  if (!seasonMarkerMatches(titleText, seasonNumber)) return true;
