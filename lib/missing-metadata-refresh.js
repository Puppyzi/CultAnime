...
import { episodeNeedsMetadataRefresh } from './episode-overview-trust';
...
function getCandidates(db, { lookbackDays, limit }) {
  ...
  const rows = db.prepare(`
    ...
    LIMIT ?
  `).all(cutoffDate(lookbackDays), cutoffDateTime(lookbackDays), queryLimit);

  return rows
    .filter(episode => episodeNeedsMetadataRefresh(episode, { status: episode.anime_status }, { lookbackDays }))
    .slice(0, limit);
}
