...
import { refreshMissingEpisodeMetadata } from './missing-metadata-refresh';

const GLOBAL_KEY = Symbol.for('cultanime.libraryReconciler');
const DEFAULT_INTERVAL_MS = 60000;
const DEFAULT_STARTUP_DELAY_MS = 5000;
const DEFAULT_ON_READ_INTERVAL_MS = 60000;
...
async function runReconcile(state, reason) {
  ...
  try {
    ...
    const missingMetadataRefresh = await refreshMissingEpisodeMetadata();
    state.lastResult = {
      ...
      local_media_prune: localPrune,
      missing_metadata_refresh: missingMetadataRefresh,
    };
    state.runCount++;
    return result;
  } catch (error) {
...
export async function reconcileForRead() {
  ...
  // Only the very first read after boot waits, so a fresh deployment can
  // populate an empty library before responding. Every later read returns
  // the current database contents immediately while the sync (already
  // covered by the background interval) continues off the request path.
  if (state.lastFinishedAt) {
    reconcile.catch(() => {});
    return { skipped: true, reason: 'background', status: publicStatus(state) };
  }
