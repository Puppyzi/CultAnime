...
function cleanPayload(payload, request) {
  return {
    ...
    clientAt: cleanText(payload?.clientAt, 80),
    queuedAt: cleanText(payload?.queuedAt, 80),
    replayed: cleanBoolean(payload?.replayed),
    ...
    online: cleanBoolean(payload?.online),
    pageHidden: cleanBoolean(payload?.pageHidden),
    visibilityState: cleanText(payload?.visibilityState, 40),
